...
| 05-04 | 04 | 2 | ACOMP-01 | — | `useOrderTracking` poll 30s + cleanup no unmount | unit | `npx vitest run apps/web/src/hooks/__tests__/useOrderTracking.test.ts` | ❌ W0 | ⬜ pending |
...
|----------|-------------|------------|-------------------|
| Badge dourado (`--color-gold`) aparece na HomeScreen quando há notificações não lidas | ACOMP-05 | Requer renderização do app e verificação visual | Acessar HomeScreen com notificações pendentes; verificar badge dourado no ícone de sino |
