...
interface ChannelSelectorProps {
  ...
  email: string
  selected: 'sms' | 'email'
  onChange: (channel: 'sms' | 'email') => void
}
 ...
 */
export function ChannelSelector({ phone, email, selected, onChange }: ChannelSelectorProps) {
  const hasPhone = phone.trim().length > 0
  const hasEmail = email.trim().length > 0

  const channels: Array<{ key: 'sms' | 'email'; label: string; icon: string; disabled: boolean }> = [
    {
      key: 'sms',
      label: 'SMS',
      icon: 'phone',
        ...
        {channels.map((ch) => {
          ...
          return (
            <button
              ...
              onClick={() => !ch.disabled && onChange(ch.key)}
              style={{
                flex: 1,
                display: 'flex',
                alignItems: 'center',
